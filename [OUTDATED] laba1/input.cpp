...
static int input_sec(const char*buf, struct INPUT_INFO*ii)
{
	...
	double Ix = 1; //осевой момент инерции и момент сопротивления изгибу
	double Wx=0;
	//char* pEnd;
	printf("%s\n", buf);
	n = sscanf(buf, "%d %lf %lf", &ind, &Ix, &Wx);
...
static int input_beam(const char*buf, struct INPUT_INFO*ii)
{
	...
	{
		fprintf(stderr, "Слишком много балок/участков!\n");
		return -6;
	}
	...
	if (n != 3)
	{
		fprintf(stderr, "Плохие параметры балки/участка: %s\n", buf);
		return -7;
	}
...
static int input_displ(const char*buf, struct INPUT_INFO*ii)
{
	...
	default: //остальные ситуации - ошибка
		fprintf(stderr, "Плохие перемещения: %s\n", buf); //вывод сообщ. об ошибке
...
static int input_force(const char*buf, struct INPUT_INFO*ii)
{
	...
	{
		fprintf(stderr, "Превышено число сил\n");
		return -10;
	}
	n = sscanf(buf, "%d %lg", &force->df, &force->f);
	if (n != 2)
	{
		fprintf(stderr, "Плохие силы/ момент: %s\n", buf);//вывод ошибки
		return -11;
	}
...
static int input_q(const char*buf, struct INPUT_INFO*ii)
{
	...
	{
		fprintf(stderr, "Ошибка ввода параметров балки %s\n", buf);//вывод ошибки
		return -12;
	}
	...
	case 1:
		...
		break;
	default:
		fprintf(stderr, "Плохие распределенные силы: %s\n", buf);//вывод ошибки
		return -13;
	}
	return 0;
}
...
int input_ndiv(const char*buf, struct INPUT_INFO*ii)
{
	...
	if (n != 1)
	{
		fprintf(stderr, "Плохие разбиения: %s", buf);
		return -14;

	}
	if (ii->ndiv_cur <= 0)
	{
		fprintf(stderr, "Плохое число текущих разбиений: %d\n", ii->ndiv_cur);
		return -15;
	}
	return 0;
}
...
int input(FILE*in, struct INPUT_INFO*ii)
{
	...
	while (fgets(buf, sizeof(buf), in))//сист функция ввода строки in - дескриптор файла
	{
		...
		default: //если текст не совпал ни один из вышеперечисленных вариантов
			fprintf(stderr, "Плохие переменные: %c\n", buf[n]); //вывод ошибок stderr %c -char
			r = -1; //признак ошибки
		}
