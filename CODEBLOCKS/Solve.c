...
static void solve_band_1(double A[MAX_NDF][MAX_BAND], double F[MAX_NDF], int n, int b)//n-реальн число степ, b=реальн ширина ленты
{
	...
	for (i = 0; i < n; i++)//цикл по строкам для исключения
	{
		...
		{
			double m = -A[i][j] / v;
			F[i + j] += F[i] * m;  //обработка правой части, начинаем со след.строки
...
static void solve_band_2(const double A[MAX_NDF][MAX_BAND], const double F[MAX_NDF],
	double Q[MAX_NDF], int n, int b)
{
	int i, j;
